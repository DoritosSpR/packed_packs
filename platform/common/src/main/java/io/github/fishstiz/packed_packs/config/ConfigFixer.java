...
class ConfigFixer {
    static File getPreviousPreferencesFile() {
        return Services.PLATFORM.getConfigDir().resolve(PackedPacks.MOD_ID + ".preferences.properties").toFile();
    }

    static File getPreviousConfigFile() {
        return Services.PLATFORM.getConfigDir().resolve(PackedPacks.MOD_ID + ".json").toFile();
    }
