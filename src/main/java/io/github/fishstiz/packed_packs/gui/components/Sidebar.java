...
public class Sidebar<E, F> extends ToggleableDialog<LayoutWrapper<LinearLayout, F>, E> {
    ...
    protected Sidebar(Builder<E, F> builder) {
        super(builder);

        this.getRoot().setMessage(builder.title);
        this.getRoot().setMinWidth(builder.minWidth);
        this.getRoot().setMetadata(builder.rootMetadata);
        ...
        header.visitWidgets(this::addWidget);
    }

    public void repositionElements() {
        this.getRoot().setMinHeight(this.screen.height);
        this.getRoot().repositionElements();
    }

    public static <E, F> Builder<E, F> builder(Screen screen) {
        return new Builder<>(screen, new LayoutWrapper<>(LinearLayout.vertical(), MIN_WIDTH, Minecraft.getInstance().getWindow().getHeight()));
    }

    public static class Builder<E, F> extends ToggleableDialog.Builder<LayoutWrapper<LinearLayout, F>, E, Builder<E, F>> {
        private Component title;
        private boolean shadow;
        private LayoutSettings headerSettings = LayoutSettings.defaults();
        private int minWidth = MIN_WIDTH;
        private F rootMetadata;

        protected Builder(Screen screen, LayoutWrapper<LinearLayout, F> root) {
            super(screen, root);
        }

        public Builder<E, F> setHeaderSettings(LayoutSettings headerSettings) {
            this.headerSettings = headerSettings;
            return this;
        }

        public Builder<E, F> setTitle(Component title) {
            this.title = title;
            return this;
        }

        public Builder<E, F> setTitle(String title) {
            this.title = Component.literal(title);
            return this;
        }

        public Builder<E, F> setTitle(Component title, boolean shadow) {
            this.shadow = shadow;
            return this.setTitle(title);
        }

        public Builder<E, F> setTitle(String title, boolean shadow) {
            this.shadow = shadow;
            return this.setTitle(Component.literal(title));
        }

        public Builder<E, F> setMinWidth(int minWidth) {
            this.minWidth = minWidth;
            return this;
        }

        public Builder<E, F> setRootMetadata(F rootMetadata) {
            this.rootMetadata = rootMetadata;
            return this;
        }

        @Override
        public Sidebar<E, F> build() {
