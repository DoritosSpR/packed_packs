...
public class ContextMenu extends ToggleableDialog<LayoutWrapper<LinearLayout>> {
    ...
    public void open(double mouseX, double mouseY, List<Option> options) {
        this.open(mouseX, mouseY, options.toArray(new Option[0]));
    }
    ...
    private static class OptionWidget extends AbstractWidget {
        ...
        OptionWidget(int width, int borderColor, ContextMenu.Option option) {
            ...
            this.separatorColor = ITheme.withAlpha(borderColor, 0.25f);
        ...
        protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
            ...
            int right = x + width;
            int bottom = y + height;
            int innerW = width - SPACING * 2;
            int innerH = height - SPACING * 2;

            if (this.isHovered()) guiGraphics.fill(x, y, right, bottom, HOVER_OVERLAY_COLOR);
